import { TTL, clearCacheData, getCache } from "@/utils/cache"
...
export async function getBaseCurrencyPriceCache(
  ...
  const baseCurrencyPrice = await getCache({
    key: "Currency_" + currency.toUpperCase(),
    execute: () => getBaseCurrencyPrice(currency),
    ttl: TTL.day1,
    isLocal: true,
  })
  // 如果时间戳为0，说明获取了错误的数据，直接取消缓存
  if (baseCurrencyPrice.time_last_update_unix === 0) {
    clearCacheData("Currency_" + currency.toUpperCase())
  }
