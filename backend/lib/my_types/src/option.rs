...
// This is needed when we would otherwise use an Option<Option<T>> in which case
// it would not be possible to tell which layer is None when represented as JSON
#[derive(
  CandidType, Serialize, Deserialize, Clone, Debug, Default, Eq, PartialEq,
)]
