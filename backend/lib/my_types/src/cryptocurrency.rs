...
#[derive(
  CandidType, Serialize, Deserialize, Clone, Debug, Eq, PartialEq, Hash,
)]
...
impl PendingCryptoTransaction {
  ...
  pub fn user_id(&self) -> Option<UserId> {
    match self {
      ...
      PendingCryptoTransaction::ICRC1(t) => {
        if t.to.subaccount.unwrap_or_default()
          == ic_ledger_types::DEFAULT_SUBACCOUNT.0
        {
  ...
  pub fn set_recipient(&mut self, owner: Principal, subaccount: Subaccount) {
    match self {
      PendingCryptoTransaction::NNS(t) => {
        t.to =
          UserOrAccount::Account(AccountIdentifier::new(&owner, &subaccount))
      }
      PendingCryptoTransaction::ICRC1(t) => {
        t.to.owner = owner;
