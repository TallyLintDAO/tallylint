...
pub struct Token {
  ...
  pub index: Nat,
  // The sha ensures that a client doesn't stream part of one version of an
  // asset followed by part of a different asset, even if not checking the
  // certificate.
...
impl HttpResponse {
  ...
  fn moved(
    ...
    let mut headers =
      vec![HeaderField("Location".to_string(), location.to_owned())];
