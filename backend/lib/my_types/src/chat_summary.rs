use crate::{
  AccessGate, BuildVersion, CanisterId, ChatId, EventIndex, EventWrapper,
  FrozenGroupInfo, GroupMember, GroupPermissions, GroupRole, HydratedMention,
  Message, MessageIndex, Milliseconds, OptionUpdate, TimestampMillis, UserId,
...
// TODO: This type is used in the response from group::public_summary and
// group_index::recommended_groups which is causing unnecessarily coupling. We
// should use separate types for these use cases. For instance we only need to
// return history_visible_to_new_joiners and is_public from
...
impl GroupCanisterGroupChatSummary {
  pub fn merge(self, updates: GroupCanisterGroupChatSummaryUpdates) -> Self {
    ...
    let mentions_to_skip = (self.mentions.len() + updates.mentions.len())
      .saturating_sub(MAX_RETURNED_MENTIONS);
    ...
    let latest_threads = updates
      ...
      // We could use Itertools `unique_by` but I didn't want to add that
      // dependency
    ...
    let membership = GroupMembership {
      ...
      mentions,
      notifications_muted: updates
        .notifications_muted
        .unwrap_or(self.notifications_muted),
    ...
    GroupCanisterGroupChatSummary {
      ...
      latest_event_index: updates
        .latest_event_index
        .unwrap_or(self.latest_event_index),
      latest_message_index: updates.latest_message_index,
      joined: self.joined,
      participant_count: updates
        .participant_count
      ...
      events_ttl_last_updated: updates
        .events_ttl_last_updated
        .unwrap_or(self.events_ttl_last_updated),
