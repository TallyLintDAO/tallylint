...
use crate::{
  CanisterId, CompletedCryptoTransaction, CryptoTransaction,
  CryptoTransferDetails, Cryptocurrency, MessageIndex, ProposalContent,
  TimestampMillis, TotalVotes, User, UserId,
};
...
const OPENCHAT_BOT_USER_ID: UserId = UserId::new(Principal::from_slice(&[
...
impl MessageContent {
  ...
  pub fn notification_text(
    &self,
    mentioned: &[User],
    user_groups_mentioned: &[(u32, String)],
  ) -> Option<String> {
    ...
    for User { user_id, username } in mentioned {
      text =
        text.replace(&format!("@UserId({user_id})"), &format!("@{username}"));
    }
  ...
  pub fn notification_image_url(&self) -> Option<String> {
    match self {
      ...
      MessageContent::Video(v) => {
        v.image_blob_reference.as_ref().map(|b| b.url())
      }
      MessageContent::Text(_)
  ...
  pub fn notification_crypto_transfer_details(
    &self,
    mentioned: &[User],
  ) -> Option<CryptoTransferDetails> {
...
impl MessageContentInitial {
  ...
  pub fn validate_for_new_group_message(
    ...
    self.validate_for_new_message(
      sender,
      false,
      forwarding,
      Some(proposals_bot_user_id),
      now,
    )
  }

  // Determines if the content is valid for a new message, this should not be
  // called on existing messages
  fn validate_for_new_message(
    ...
    if forwarding {
      match self {
        ...
        | MessageContentInitial::Deleted(_) => {
    ...
    // Allow GovernanceProposal messages to exceed the max length since they are
    // collapsed on the UI
    } else if self.text_length() > MAX_TEXT_LENGTH_USIZE
      && !matches!(self, MessageContentInitial::GovernanceProposal(_))
    {
  ...
  pub fn text(&self) -> Option<&str> {
    match self {
      ...
      MessageContentInitial::Deleted(_) | MessageContentInitial::Custom(_) => {
        None
      }
    }
  }
...
impl From<MessageContent> for MessageContentInitial {
  fn from(content: MessageContent) -> Self {
    match content {
      ...
      MessageContent::GovernanceProposal(c) => {
        MessageContentInitial::GovernanceProposal(c)
      }
      ...
      MessageContent::Prize(_) => {
        panic!("Cannot convert output prize to initial prize")
      }
      MessageContent::PrizeWinner(_) => {
        panic!("Cannot send a prize winner message")
      }
      MessageContent::MessageReminderCreated(r) => {
        MessageContentInitial::MessageReminderCreated(r)
      }
      MessageContent::MessageReminder(r) => {
        MessageContentInitial::MessageReminder(r)
      }
      MessageContent::ReportedMessage(_) => {
        panic!("Cannot send a 'reported message' message")
      }
...
impl BlobReference {
  pub fn url(&self) -> String {
    format!(
      "https://{}.raw.icp0.io/files/{}",
      self.canister_id, self.blob_id
    )
  }
}
