use crate::{
  AccessGate, ChannelId, CommunityPermissions, CommunityRole, EventIndex,
  EventWrapper, GroupPermissions, GroupRole, Message, MessageIndex,
  ...
  pub fn split(
    ...
    for event_or_expired_range in events_and_expired_ranges {
      match event_or_expired_range {
        ...
        EventOrExpiredRange::ExpiredEventRange(from, to) => {
          expired_ranges.push((from, to))
        }
      }
    }
