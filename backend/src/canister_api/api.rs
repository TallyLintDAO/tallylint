...
// build)没有告警，但是生成发布文件(cargo build --release)的时候继续告警，
#![cfg_attr(
  debug_assertions,
...
pub async fn create_and_install() -> String {
  ...
  //  CallResult<(CanisterIdRecord,)> : Result----unwrap  ()----tuple .0 to get
  // the first tuple.
  #[warn(unused_mut)]
  ...
  //   "install_code error:  message Only the controllers of the canister
  // avqkn-guaaa-aaaaa-qaaea-cai can control it.\nCanister\'s controllers:
